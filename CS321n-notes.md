...
![image-20251108234430733](F:\王梓恒\学习资料\Machine_Learning\Deep_Learning\CV\CS231n\images\image-20251108234430733.png)

注意到每层运算之后表征图像的Tensor的二维大小是在缩水的，解决方式就是 ==**Padding**==，再周围添加额外的虚拟数据 0
...
Attention机制是由循环神经网络RNN引申而来的，先看一个使用RNN翻译的例子

![image-20251107224415285](F:\王梓恒\学习资料\Machine_Learning\Deep_Learning\CV\CS231n\images\image-20251107224415285.png)

​	c 代表 context，是总结整个输入序列得到的向量，即**上下文向量**，一般就是==**输入的最后一个隐藏状态，包含了整个输入序列的信息**==

​	但这种结构存在 encoder 和 decoder 的通信瓶颈，因为二者之间仅依靠一个**固定长度的向量** c 传递信息
当输入序列较短时问题不大，但是当要翻译的是一整篇文章/一整本书，要求把输入序列的所有信息压缩为单个向量是很不合理的

​	Intuitive solution：不要用一个固定长度的向量来限制模型能力，而是让模型在生成输出序列时不断==**“回看”**==整个输入序列

### Sequence to Sequence with RNNs and Attention

首先 encoder 保持不变，通过输入序列最终的隐藏状态$h_t$通过某种方法得到初始的 decoder 隐藏状态$s_0$

然后加入 Attention 机制：
	① 通过**线性层**$f_{att}$计算输入隐藏状态$h_i$与输出隐藏状态$s_t$的 alignment scores $e_{t,i}=f_{att}(s_{t-1},h_i)$，$e_{t,i}$ 是一个**标量**，表征$h_i$所对应的 input token $x_i$ 与$s_t$的==**相似程度**==
	② alignment scores 过一层 softmax 得到**注意力权重** $a_{t,i}$，==**输入序列各隐藏状态通过注意力权重加权求和得到上下文向量 $c_t$**==，注意力权重$a_{t,i}$的大小可以表征输出隐藏状态$s_t$更*倾向于*回顾哪些输入隐藏状态
	③  然后对于输出序列的每个隐藏状态都重复此步骤

![image-20251107231156338](F:\王梓恒\学习资料\Machine_Learning\Deep_Learning\CV\CS231n\images\image-20251107231156338.png)

![image-20251107231702010](F:\王梓恒\学习资料\Machine_Learning\Deep_Learning\CV\CS231n\images\image-20251107231702010.png)

​	No supervision，then how to calculate loss？
​	训练样本是大量的 input-output pairs，只是没有显式监督序列不同token之间的关系

​	我们可以通过观察训练过程中的注意力权重大小来尝试理解网络在解决问题时的“思路”，注意力机制为我们提供了一种理解神经网络处理过程的方法  ==**可解释性**==

![image-20251107233015194](F:\王梓恒\学习资料\Machine_Learning\Deep_Learning\CV\CS231n\images\image-20251107233015194.png)

​	前面引出 Attention 是从“修复RNN”的角度出发的，但这个机制非常有趣，我们尝试把注意力机制提取出来，使其不依赖RNN也能作为网络结构中独立的模块 (computational primitive)

​	所以要尝试把前一个 RNN+Attention 的例子中注意力机制的核心功能抽象出来

### Attention Layer

![image-20251107233756695](F:\王梓恒\学习资料\Machine_Learning\Deep_Learning\CV\CS231n\images\image-20251107233756695.png)     	

​	从之前的例子中只关注注意力机制，就可以提取出下图中的 Attention Layer
​	注意力层不断重复这样的流程：① 取一个 **query vector**；② 与所有的 **date vector** 进行函数运算得到 similarities；③ 过一层 softmax 得到 **attention weights**；④ 与 **data vector** 加权运算得到 **output vector**；⑤ 再取一个 **query vector**，重复上述流程
​	至于如何获取 query vector？注意力层的核心机制并不关心这个问题

![image-20251108214141997](F:\王梓恒\学习资料\Machine_Learning\Deep_Learning\CV\CS231n\images\image-20251108214141997.png)

 	做一系列扩展操作，得到 **Cross-Attention Layer**：
	① 使用“两个向量得到一个标量”的最简单运算——点积作为计算 similarities 的方法
	② 为了**避免梯度消失**，对 similarities 做一个放缩 $E=QK^T/\sqrt{D_Q}$   $\color{red} 但没太懂原理 \color{red}$ 
	③ 扩展到批量 query vector 处理
	④ 在计算 similarities 和 计算 output vector 两个不同的步骤都是用 data vectors 是很奇怪的，==于是增加两个映射矩阵，把 data vectors 映射为 **Key** 和 **Value**，分别用于两次运算==；可以用一个形象的比喻来理解这一过程：想象在浏览器中搜索一个问题，你输入的内容就是 query vector，互联网上的所有数据就是 data vectors，要根据 Key 搜索哪些 data 是与 query 最匹配的，然后根据 Value 决定具体输出哪些内容作为回复 

![image-20251108222759775](F:\王梓恒\学习资料\Machine_Learning\Deep_Learning\CV\CS231n\images\image-20251108222759775.png)

​	上述情况的注意力层有 query 和 data 两种输入，它们可以来自完全不同的源头，所以叫 **Cross-Attention Layer**；但是当只有一种输入 **Input Vectors** 的时候，把它映射到三种语义的矩阵，就可以形成 **Self-Attention Layer**

![image-20251108225742312](F:\王梓恒\学习资料\Machine_Learning\Deep_Learning\CV\CS231n\images\image-20251108225742312.png)

​	当输入向量的顺序被打乱时，注意力层的输出会发生什么呢？**输出的内容完全相同，但按相同规则打乱**
​	这种性质被称为 *permulation equivariant 置换不变性* 

![image-20251108232311912](F:\王梓恒\学习资料\Machine_Learning\Deep_Learning\CV\CS231n\images\image-20251108232311912.png)

​	但这其实会引发一些问题，有时候我们需要告诉网络输入向量的顺序关系，因此需要附加额外的数据片段；此外还有一些注意力层的技巧，比如通过加掩码强制某些输出不受某些输入影响，比如**多头注意力机制 Multihead Self-Attention** 
![image-20251108233410864](F:\王梓恒\学习资料\Machine_Learning\Deep_Learning\CV\CS231n\images\image-20251108233410864.png)

![image-20251108233518242](F:\王梓恒\学习资料\Machine_Learning\Deep_Learning\CV\CS231n\images\image-20251108233518242.png)

整个多头注意力层的本质就是四次矩阵乘法：

![image-20251108233645667](F:\王梓恒\学习资料\Machine_Learning\Deep_Learning\CV\CS231n\images\image-20251108233645667.png)

总结一下我们目前学过的三种处理输入序列的方法：

![image-20251108234626891](F:\王梓恒\学习资料\Machine_Learning\Deep_Learning\CV\CS231n\images\image-20251108234626891.png)

### The Transformer

![image-20251108235051360](F:\王梓恒\学习资料\Machine_Learning\Deep_Learning\CV\CS231n\images\image-20251108235051360.png)

![image-20251108235127507](F:\王梓恒\学习资料\Machine_Learning\Deep_Learning\CV\CS231n\images\image-20251108235127507.png)

## Lec9 Detection, Segmentation, Visualization and Understanding
